...
getwd()
directorio = "/Users/stefa/Documents/Code/Voteview_Plots/" # Workspace
setwd(directorio)
dir.create("Graphs")
...
# Puntos ideales / Bliss Points # DATA FOR ROLLCALLS #
{
print("tranqui tarda un poco, prepara el mate")
res_pre2GM = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                              startdate = "1935-01-20", enddate = "1939-01-20")
res_dur2GM = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                              startdate = "1940-01-20", enddate = "1945-01-20")
res_post2GM = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                               startdate = "1946-01-20", enddate = "1950-01-20")
...
} # Utiliza la sección "codes.Clausen" para filtrar la categoría elegida en los años elegidos #
...
# Busca los "legisladores extremos" // Variable Polaridad #
{
...
summary(rc_pre2GM, verbose = TRUE)

cons1_pre2GM; cons2_pre2GM # Candidatos extremos pre 2GM
cons1_dur2GM; cons2_dur2GM # Candidatos extremos durante 2GM
cons1_post2GM; cons2_post2GM # Candidatos extremos post 2GM


# Crea las labells de partidos y grafica #
{
  defIdeal_pre2GM$legislators$partyName = ifelse(defIdeal_pre2GM$legislators$party == 200, "Republican",
                                                  ifelse(defIdeal_pre2GM$legislators$party == 100, "Democrat", "Independent"))
  graph_pre2GM = ggplot(defIdeal_pre2GM$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Previo a la 2GM") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/pre_2GM.jpg", plot = graph_pre2GM, width = 8, height = 6, dpi = 300)
  
  defIdeal_dur2GM$legislators$partyName = ifelse(defIdeal_dur2GM$legislators$party == 200, "Republican",
                                                  ifelse(defIdeal_dur2GM$legislators$party == 100, "Democrat", "Independent"))
  graph_dur2GM = ggplot(defIdeal_dur2GM$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Durante la 2GM") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/dur_2GM.jpg", plot = graph_dur2GM, width = 8, height = 6, dpi = 300)
  
  defIdeal_post2GM$legislators$partyName = ifelse(defIdeal_post2GM$legislators$party == 200, "Republican",
                                                   ifelse(defIdeal_post2GM$legislators$party == 100, "Democrat", "Independent"))
  graph_post2GM = ggplot(defIdeal_post2GM$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Luego de la 2GM") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/post_2GM.jpg", plot = graph_post2GM, width = 8, height = 6, dpi = 300)
...
res_preV = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                            startdate = "1955-01-20", enddate = "1964-01-20")
res_postV = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                             startdate = "1965-01-20", enddate = "1975-01-20")
res_durV = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                            startdate = "1976-01-20", enddate = "1985-01-20")
...
# Graficos #
{
  defIdeal_preV$legislators$partyName = ifelse(defIdeal_preV$legislators$party == 200, "Republican", 
                                                ifelse(defIdeal_preV$legislators$party == 100, "Democrat", "Independent"))
  graph_preV = ggplot(defIdeal_preV$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Previo a la Guerra de Vietnam") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/pre_V.jpg", plot = graph_preV, width = 8, height = 6, dpi = 300)
  
  defIdeal_durV$legislators$partyName = ifelse(defIdeal_durV$legislators$party == 200, "Republican", 
                                                ifelse(defIdeal_durV$legislators$party == 100, "Democrat", "Independent"))
  graph_durV = ggplot(defIdeal_durV$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Durante la Guerra de Vietnam") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/dur_V.jpg", plot = graph_durV, width = 8, height = 6, dpi = 300)
  
  defIdeal_postV$legislators$partyName = ifelse(defIdeal_postV$legislators$party == 200, "Republican", 
                                                 ifelse(defIdeal_postV$legislators$party == 100, "Democrat", "Independent"))
  graph_postV = ggplot(defIdeal_postV$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Luego de la Guerra de Vietnam") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/post_V.jpg", plot = graph_postV, width = 8, height = 6, dpi = 300)
}
...
# Puntos ideales #
{
  ...
  res_preA = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                              startdate = "1995-01-20", enddate = "2000-01-20")
  res_postA = voteview_search("codes.Clausen:Foreign and Defense Policy support:[15 to 85]",
                               startdate = "2001-01-20", enddate = "2005-01-20")
...
{
  defIdeal_preA$legislators$partyName = ifelse(defIdeal_preA$legislators$party == 200, "Republican", 
                                                ifelse(defIdeal_preA$legislators$party == 100, "Democrat", "Independent"))
  graph_preA = ggplot(defIdeal_preA$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Previo a la invasión de Afganistan") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/pre_A.jpg", plot = graph_preA, width = 8, height = 6, dpi = 300)
  
  defIdeal_postA$legislators$partyName = ifelse(defIdeal_postA$legislators$party == 200, "Republican", 
                                                 ifelse(defIdeal_postA$legislators$party == 100, "Democrat", "Independent"))
  graph_postA = ggplot(defIdeal_postA$legislators, aes(x = coord1D, y = coord2D, color = partyName, label = state_abbrev)) +  
    geom_text(size = 3) + 
    scale_color_manual("Party", values = c("Republican" = "red", "Democrat" = "blue", "Independent" = "yellow")) + 
    theme_bw() + 
    labs(x = "Economics", y = "Social", title = "Luego de la invasión de Afganistan") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.x = element_text(size = 12),
          axis.title.y = element_text(size = 12))
  
  ggsave(filename = "Graphs/post_A.jpg", plot = graph_postA, width = 8, height = 6, dpi = 300)
}
...
##############################################################################################################
##############################################################################################################
# OTHER CLAUSEN CATEGORIES YOU MAY BE ABLE TO TEST
